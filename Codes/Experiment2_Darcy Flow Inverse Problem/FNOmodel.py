...
class CMProjectionNeumann1d(nn.Module):
    ...
    def __init__(self, Lx: float, alpha: float, tau: float, min_eig: float = 1e-12):
        super().__init__()
        self.Lx = float(Lx)
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        mult = lamA.pow(-0.5 * self.alpha)       # (Nx,)
...
class CosineFNO1d(nn.Module):
    ...
    def __init__(
        ...
        self.cm = (
            CMProjectionNeumann1d(Lx, alpha=cm_alpha, tau=cm_tau)
            if use_cm
            else nn.Identity()
        )
...
class MultiInputCosineFNO1d(nn.Module):
    ...
    def __init__(self, base_fno: nn.Module,
        ...
        super().__init__()
        self.fno = base_fno   # e.g., CosineFNO1d(Cin=3, Cout=1, ...)
    ...
    def forward(self, u: torch.Tensor, y: torch.Tensor, v: torch.Tensor) -> torch.Tensor:

        # --- normalize ---
        u_raw = u
        u = (u - self.mean_u) / self.std_u
        y = (y - self.mean_y) / self.std_y
        v = (v - self.mean_y) / self.std_y

        # Basic shape checks
        assert u.dim() == y.dim() == v.dim() == 3, "Use (B,C,Nx) tensors."
        assert u.shape[1] == y.shape[1] == v.shape[1] == 1, "Each input must be 1 channel."
        ...
        v_up = spectral_upsample_neumann_1d(v, Nx_u)   # (B,1,Nx_u)

        # Concatenate channels → 3-channel signal
        x = torch.cat([u, y_up, v_up], dim=1)          # (B,3,Nx_u)

        # FNO backbone + identity (residual)
        out = u_raw + self.fno(x)                      # (B,1,Nx_u)
        return out

class FNOTripletAdapter1d(nn.Module):
    """
    Adapter for 1D FNO pipelines.

    Maps flattened vectors (u, y, ydag) of shape:
        u_vec    : (N, Nx_u)
        y_vec    : (N, Nx_y)
        ydag_vec : (N, Nx_y)
    ...
    Applies:
        out_img = fields(u_img, y_img, ydag_img)
    ...
    ----------
    base_fno  : nn.Module
        A CosineFNO1d model with Cin=3, Cout=1.
    mean_u, std_u : float
        Normalization stats for u.
    mean_y, std_y : float
        Normalization stats for y, ydag.
    """
    ...
    def forward(self, u_vec, y_vec, ydag_vec):
        """
        u_vec    : (N, Nx_u)
        y_vec    : (N, Nx_y)
        ydag_vec : (N, Nx_y)
        """
        self._maybe_set_shapes(u_vec, y_vec)

        N = u_vec.size(0)

        # Reshape into 1D fields: (N, 1, Nx)
        u_img    = u_vec.reshape(N, 1, self.Nx_u)
        y_img    = y_vec.reshape(N, 1, self.Nx_y)
        ydag_img = ydag_vec.reshape(N, 1, self.Nx_y)

        # Multi-input FNO
        out_img = self.fields(u_img, y_img, ydag_img)   # (N,1,Nx_u)
...
# def test_dct_roundtrip_1d():
#     print("=== TEST 1 (1D): DCT round-trip ===")

#     x = torch.randn(4, 1, 128)    # Shape (B, C, N), last axis = DCT axis
#     xr = idct1(dct1(x))
...
# def test_zero_mode_removed_1d():
#     print("=== TEST 2 (1D): Zero-mode removal ===")

#     B, N = 2, 256
#     const = torch.ones(B, 1, N)

#     cm = CMProjectionNeumann1d(Lx=1.0, alpha=1.0, tau=1.0)
#     out = cm(const)

#     m = out.mean().item()
#     print("mean after CM =", m)

#     assert abs(m) < 1e-6
#     print("✅ PASSED\n")


# def test_spectral_filter_correctness_1d():
#     print("=== TEST 3 (1D): Spectral filter correctness ===")

#     B, N = 1, 256
#     alpha = 1.1
#     tau = 0.8

#     cm = CMProjectionNeumann1d(Lx=1.0, alpha=alpha, tau=tau)
#     z = torch.randn(B, 1, N)
...
#     )
#     lamA = lam + tau**2
#     lamA = torch.clamp(lamA, min=cm.min_eig)
#     mult = lamA.pow(-0.5 * alpha)         # spectral weight

#     Z_manual = Z * mult
#     Z_manual[..., 0] = 0.0                # remove constant mode

#     diff = (Z_manual - out_dct).abs().max().item()
#     print("max spectral diff =", diff)

#     assert diff < 1e-5
#     print("✅ PASSED\n")


# def test_linearity_1d():
#     print("=== TEST 4 (1D): Linearity ===")
...
#     print("linearity error =", err)

#     assert err < 1e-6
#     print("✅ PASSED\n")


# def test_shape_preserving_1d():
#     print("=== TEST 5 (1D): Shape preservation ===")
...
# -------------------------------------------
# DEMO: Rough function + CM projection
# -------------------------------------------

# if __name__ == "__main__":
#     test_dct_roundtrip_1d()
#     test_zero_mode_removed_1d()
#     test_spectral_filter_correctness_1d()
#     test_linearity_1d()
#     test_shape_preserving_1d()

#     print("===============================================")
#     print("  ✅ ALL 1D CM / DCT TESTS PASSED SUCCESSFULLY")
#     print("===============================================")
