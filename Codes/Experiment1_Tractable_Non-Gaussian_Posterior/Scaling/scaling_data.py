#!/usr/bin/env python3
# ============================================================
# scaling_models_data_two_losses.py
#
# Runs the SAME data-scaling sweep for TWO losses:
...
# for each (width, depth) model config.
#
# Everything saved under:
#   ./data_scaling_saved_<RUN_TAG>/
#       meta.json
#       scaling_models_data_two_losses.py (copy)
...
# Data scaling grid + seeds
K_TRAINS = [500, 1000, 2000, 4000, 8000, 16000, 32000, 64000]
# K_TRAINS = [500, 1000]
SEEDS    = [0, 1, 2]
# SEEDS    = [0, ]

# Two losses to compare
LOSS_MODES = ["joint", "prior"]  # "joint" uses 3D input, "prior" uses 2D input
...
NY_KAPPA   = 30
NP_PUSH    = 5000
PUSH_BATCH = 1024    # <= memory-safe chunk for pushforwards
U_SPAN     = 20.0
N_GRID     = 4001
...
# -------------------------
# Device + output folders (timestamp)
# -------------------------
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
RUN_TAG = time.strftime("%Y%m%d_%H%M%S")
OUT_DIR = f"./data_scaling_saved_{RUN_TAG}"
...
try:
    shutil.copy(__file__, os.path.join(OUT_DIR, "scaling_models_data_two_losses.py"))
...
# -------------------------
# Model (supports 2D or 3D input depending on mode)
# -------------------------
...
# -------------------------
# OOM-safe repulsion helper (sorting trick)
# -------------------------
def mean_abs_pairwise_torch(v):
    ...
    pair_sum = torch.sum(v_sorted * (2*k - B - 1))  # sum_{i<j}(v_j - v_i)
...
def energy_distance_loss(model, u_batch, y_batch):
    """
    JOINT loss (your updated version idea but kept OOM-safe in repulsion):
      - Accuracy excludes diagonal
      - Repulsion averages over k of mean_{i,j} |T_{i,k} - T_{j,k}|
        computed OOM-safe per column via sorting trick.
    """
    ...
    # Repulsion term: for each column k, compute mean_{i,j}|T_{i,k}-T_{j,k}|
...
def energy_distance_loss_fromprior(model, u_batch, y_batch):
    """
    PRIOR loss (your updated structure, OOM-safe repulsion):
    ...
    acc_pred = model(u_i, y_j)  # y_j plays role of y_dag
    acc_matrix = torch.abs(acc_pred - u_j).view(B, B)
    acc_matrix.fill_diagonal_(0.0)
    acc_loss = acc_matrix.sum() / (B * (B - 1))

    # Repulsion term: for each y_k column, compute mean_{i,j}|T_{i,k}-T_{j,k}|
    y_k = y_batch.view(1, B).expand(B, B)
    u_i_mat = u_batch.view(B, 1).expand(B, B)
...
# -------------------------
# PUSHFORWARD sampler (BATCHED) + evaluation
# -------------------------
@torch.no_grad()
def pushforward_samples_for_y_batched(
    ...
    """
    Memory-safe pushforward.
    For loss_mode="joint": sample (u_i, y_i) from base and compute T(u_i, y_i; y_dag)
    For loss_mode="prior": sample u_i from base and compute T(u_i; y_dag) (y_base unused)
    Returns (Np,) numpy array.
    """
    n = len(u_base)
...
# -------------------------
# Train one run for a given (loss_mode, model config, K_train) (SAVES best ckpt)
# -------------------------
...
# -------------------------
# Run DATA scaling sweep for MULTIPLE models and BOTH losses
# -------------------------
def run_models_data_scaling_with_avgED(
    loss_mode,
    u_train_pool, y_train_pool,
    u_val, y_val,
    u_test, y_test,
    ...
    for cfg in model_configs:
        ...
        for K in K_trains:
            ...
            print(
                f"[{loss_mode}] (w={hidden_dim}, d={num_layers}, K={K}) "
...
def plot_models_data_scaling_two_losses(all_results_by_loss, split="test", logx=True, logy=True):
    """
    Same color = same (width, depth)
    Solid = joint
    Dashed = prior
    Scaling line = black
    """
    ...
    # --------------------------------------------------
    # 1) Collect ALL unique (width, depth) pairs
    # --------------------------------------------------
    unique_configs = set()
    ...
    # --------------------------------------------------
    # 2) Assign ONE color per (width, depth)
    # --------------------------------------------------
    ...
    for loss_mode in sorted(all_results_by_loss.keys()):
        ...
        for (w, d), rows in sorted(groups.items(), key=lambda t: (t[0][1], t[0][0])):
            ...
            plt.errorbar(
                ...
                capsize=3,
                linestyle=linestyle,
                color=color,          # <- SAME COLOR FOR SAME MODEL
                label=label
            )

    # --------------------------------------------------
    # 4) Add 1/sqrt(K) reference line (BLACK)
    # --------------------------------------------------
    if ref_Ks is not None:
        _, ref = _add_inv_sqrt_reference(ref_Ks, ref_ys)
        if ref is not None:
            plt.plot(
                ref_Ks,
                ref,
                linestyle=":",
                color="black",        # <- BLACK scaling line
                linewidth=2,
...
def main():
    ...
    # ---- FIXED SIZE splits: TEST=1000, VAL=1000 ----
    ...
    # fixed y~kappa for VAL and TEST (same for both losses)
    ...
    for loss_mode in LOSS_MODES:
        results = run_models_data_scaling_with_avgED(
            loss_mode=loss_mode,
            u_train_pool=u_train_pool, y_train_pool=y_train_pool,
            u_val=u_val, y_val=y_val,
    ...
    combined_path = os.path.join(OUT_DIR, "results_models_data_scaling_two_losses.json")
    with open(combined_path, "w") as f:
        json.dump(all_results_by_loss, f, indent=2)

    # plots (one figure for VAL, one for TEST), each overlays both losses
    plot_models_data_scaling_two_losses(all_results_by_loss, split="val",  logx=True, logy=True)
    plot_models_data_scaling_two_losses(all_results_by_loss, split="test", logx=True, logy=True)
